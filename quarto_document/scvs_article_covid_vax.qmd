...
library(ggfixest) # ggplot2 for fixest
...
# Here we write the code for our regressions to be presented in the tables in the document. 
# We cluster by province and by month.

# Models with total business creation ---------------------------------------------------------------------------------------

# Simple one, only fixed effects + did estimator.
...
# With vaccination rate (at least one dose)
...
# Including a trend term to correct for violation of parallel trends
...
# Models with non-SAS business creation ------------------------------------------------------------------------------------
...
#| label: regressions_table1
#| output: asis

# To produce the table that I used in the actual paper, I created a dataframe of the model results and exported to Excel to include an edited table. I produce the XL table that I used in the paper in the edited_reg_table.xlsx file. 

# I comment out the code to produce the XL file here here, and include a markdown table which is a bit simpler in formatting.  
...
# I also haven't been able to produce a horizontal page layout for some pages in Word. This would only work in PDF. Thus, the table looks a bit shitty in the document at the end. 

# We construct the map of coefficient names

coefficient_names<-c('(Intercept)' = 'Constant',
                     'did' = 'Treatment x Post', 
                     'excessd_rate' = 'Excess Deaths Rate',
                     'job_change' = 'Job Contracts Variation',
                     'v_deaths' = 'Homicides',
                     'robbery' = 'Thefts',
                     'transit_acc' = 'Transit Accidents',
                     'vax_leastone' = 'Vaccination Rate',
                     'time_cont' = 'Time Trend') 
...
```{r}
#| label: robustness-checks-wrangling

# Drop Pichincha and Guayas

# Some base R, just for fun:

df_smallprov<-
  df[!(df$province == 'PICHINCHA' | df$province == 'GUAYAS'),]

# Mistreatment 1: Only pichincha as treatment

df_mistreat<-df

df_mistreat$pich<-
  ifelse(df_mistreat$province == 'PICHINCHA',1,0)

df_mistreat$period

df_mistreat<-
  df_mistreat %>%
  mutate(pich_treat = pich*post_dummy)

```
```{r}
#| label: robustness-checks-models

# Here we estimate the models for the robustness checks. We use the time-trends controlled model.
...
# List of robustness check regressions

rob_regressions_list<-list(nopich_lbuss,
                           nopich_lnsas,
                           pich_lbuss,
                           pich_lnsas)
```
```{r}
# |label: robustness-checks-models-table
# |include: true
# |output : asis

# Here I do the same stuff as I did before: I only include a table on the shittier side-the original one I did in Excel to comply with the journal rules

# Thus, the reader of this code should assume that the number of these models include an asterisk, to match the text.

modelsummary(rob_regressions_list,
             output = 'markdown',
             title = 'Baseline Regression Results',
             coef_map = coefficient_names,
             stars = c('*' = .1, '**'= 0.05, '***' = .01))
...
```{r}
#| label: prior-trends-graph
#| include: false

# This chunk creates the typical prior trends graph.
# I first load the data for the trends, which I created with basis to the the SCVS data. 

ptrends_graph_df<-read.csv('../data/ptrends_graph.csv')

# I do two graphs, one with non-sas and the other one with totals. 
...
```{r}
#| label: p-trends-graph-print
#| fig-width: 10
#| fig-height: 5
#| fig-cap: 'Prior trends for the natural logarithm of total and non-SAS business creation'
...
df_prev$monthyear<-as.factor(df$time_cont)

df_prev$monthyear<-relevel(df$monthyear, '20')

# Run the test regressions

test_lbuss <-
  feols(lbuss ~ i(time_cont, treat_dummy, ref = 20) + excessd_rate + job_change + v_deaths + robbery + transit_acc,
        fixef = c("province", "month"),
        cluster = ~ province + month,
        data = df)
...
test_lnsas<-
  feols(lnsas ~ i(time_cont, treat_dummy, ref = 20) + excessd_rate + job_change + v_deaths + robbery + transit_acc,
        fixef= c("province", "month"),
        cluster = ~ province + month,
        data = df)
...
modelsummary(list(test_lbuss, test_lnsas),
             output = 'dataframe') %>% write.xlsx("../tables/coefplot_nuevo.xlsx")

modelsummary(test_lnsas_prev,
             output = "dataframe") %>% write.xlsx('../tables/coefplot_antiguo.xlsx')
...
```
```{r}
#| label: graphs-prior-trends-regression
#| include: false
...
```
```{r}
#| label: p-trends-reg-graph-print
#| fig-width: 10
#| fig-height: 5
#| fig-cap: 'Prior trends for the natural logarithm of total and non-SAS business creation'
...
# Appendix: Wild Bootstrap Tests for Baseline Regression Models

```{r}
#| label: wild-bootstraps
#| include: false

# Here, I try to run the bootstrap tests for my regressions
...
# Sadly, these boottests cannot be run by R for some reason. I have attached the Stata do-files which have these included. The results table in the paper are probably different to what you see as results from the do-file, but that is because of the simulation. 

# The following code is what I used to produce the table in XL. I then edited it from Word to end with the final result: 
